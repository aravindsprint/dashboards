...
<div id="app">
  ...
  <div class="sd-header">
    ...
    <div class="sd-header__right">
      ...
      <button class="sd-btn" @click="exportExcel">⬇ Excel</button>
      <button class="sd-btn" @click="exportPdf">⬇ PDF</button>
        ...
        <div class="sd-twrap">
          <table class="sd-table">
            <thead>
              ...
              <tr class="sd-filter-row">
                <th><input class="sd-colf" v-model="colFilters.name_color" placeholder="Filter…"/></th>
                <th><input class="sd-colf" v-model="colFilters.width" placeholder="Filter…"/></th>
                <th v-if="activeTab!=='mars200'"><input class="sd-colf" v-model="colFilters.item_code" placeholder="Filter…"/></th>
                <th v-if="activeTab==='mars200'"><input class="sd-colf" v-model="colFilters.batch_no" placeholder="Filter…"/></th>
                <th v-else><input class="sd-colf" v-model="colFilters.warehouse" placeholder="Filter…"/></th>
                <th v-if="activeTab==='mars200'"><input class="sd-colf" v-model="colFilters.parentwarehouse" placeholder="Filter…"/></th>
                <th><input class="sd-colf sd-colf-r" v-model="colFilters.qty" placeholder="Filter…"/></th>
                <th v-if="activeTab!=='mars200'"><input class="sd-colf" v-model="colFilters.batch_no" placeholder="Filter…"/></th>
                <th><input class="sd-colf" v-model="colFilters.batch_status" placeholder="Filter…"/></th>
                <th v-if="activeTab==='mars200'"><input class="sd-colf sd-colf-r" v-model="colFilters.collar_qty" placeholder="Filter…"/></th>
                <th v-if="activeTab==='mars200'"><input class="sd-colf" v-model="colFilters.collar_status" placeholder="Filter…"/></th>
                <th v-if="activeTab==='mars200'"><input class="sd-colf sd-colf-r" v-model="colFilters.cuff_qty" placeholder="Filter…"/></th>
                <th v-if="activeTab==='mars200'"><input class="sd-colf" v-model="colFilters.cuff_status" placeholder="Filter…"/></th>
              </tr>
            </thead>
            <tbody>
...
export default {
  data(){
    return {
      ...
      search: '',
      colFilters: {
        name_color: '', width: '', item_code: '', warehouse: '', batch_no: '',
        qty: '', batch_status: '', parentwarehouse: '',
        collar_qty: '', collar_status: '', cuff_qty: '', cuff_status: '',
      },
    }
  },
  computed: {
    ...
    filteredRows(){
      ...
      const matches = (val, filterVal) => (val || '').toString().toLowerCase().includes(filterVal.trim().toLowerCase());

      if(has(cf.name_color)) rows = rows.filter(r => matches(r.commercial_name, cf.name_color) || matches(r.color, cf.name_color));
      if(has(cf.width))            rows = rows.filter(r => matches(r.width, cf.width));
      if(has(cf.item_code))        rows = rows.filter(r => matches(r.item_code, cf.item_code));
      if(has(cf.warehouse))        rows = rows.filter(r => matches(r.warehouse, cf.warehouse));
      if(has(cf.batch_no))         rows = rows.filter(r => matches(r.batch_no, cf.batch_no));
      if(has(cf.batch_status))     rows = rows.filter(r => matches(r.batch_status, cf.batch_status));
      if(has(cf.parentwarehouse))  rows = rows.filter(r => matches(r.parentwarehouse, cf.parentwarehouse));
      if(has(cf.collar_status))    rows = rows.filter(r => matches(r.collar_status, cf.collar_status));
      if(has(cf.cuff_status))      rows = rows.filter(r => matches(r.cuff_status, cf.cuff_status));
      if(has(cf.qty))              rows = rows.filter(r => matches(this.fmt(r.actual_qty), cf.qty));
      if(has(cf.collar_qty))       rows = rows.filter(r => matches(this.fmt(r.collar_qty), cf.collar_qty));
      if(has(cf.cuff_qty))         rows = rows.filter(r => matches(this.fmt(r.cuff_qty), cf.cuff_qty));
  ...
  methods: {
    ...
    switchTab(key){
      this.activeTab = key;
      this.search = '';
      for(const k in this.colFilters) this.colFilters[k] = '';
      this.expanded.clear();
    ...
    // ── Export: both operate on filteredRows — i.e. whatever's currently
    // visible after the search box + column filters, not the full unfiltered set.
    _exportRows(){
      const isMars = this.activeTab === 'mars200';
      return this.filteredRows.map(r => {
        const base = {
          'Commercial Name': r.commercial_name || '',
          'Color': r.color || '',
          'Width': r.width || '',
        };
        if(!isMars) base['Item Code'] = r.item_code || '';
        if(isMars) base['Batch No'] = r.batch_no || '';
        base[isMars ? 'Warehouse Group' : 'Warehouse'] = isMars ? r.parentwarehouse : r.warehouse;
        base['Qty'] = +r.actual_qty || 0;
        if(!isMars) base['Batch No'] = r.batch_no || '';
        base['Batch Status'] = r.batch_status || '';
        if(isMars){
          base['Collar Qty'] = +r.collar_qty || 0;
          base['Collar Status'] = r.collar_status || '';
          base['Cuff Qty'] = +r.cuff_qty || 0;
          base['Cuff Status'] = r.cuff_status || '';
        }
        return base;
      });
    },
    _exportFilename(ext){
      const tabLabel = (this.tabs.find(t=>t.key===this.activeTab) || {}).label || this.activeTab;
      const stamp = new Date().toISOString().slice(0,10);
      return `inventory-${tabLabel.toLowerCase().replace(/[^a-z0-9]+/g,'-')}-${stamp}.${ext}`;
    },
    exportExcel(){
      ...
      XLSX.utils.book_append_sheet(wb, ws, (this.tabs.find(t=>t.key===this.activeTab)||{}).label || 'Sheet1');
      XLSX.writeFile(wb, this._exportFilename('xlsx'));
    },
    exportPdf(){
      const rows = this._exportRows();
      if(!rows.length){ alert('No rows to export — adjust your filters.'); return; }
      ...
      doc.setFontSize(12);
      doc.text(`Inventory Dashboard — ${(this.tabs.find(t=>t.key===this.activeTab)||{}).label || this.activeTab}`, 24, 28);
      autoTable(doc, {
        head: [headers],
        body,
        startY: 40,
      ...
      doc.save(this._exportFilename('pdf'));
    },
  },
  mounted(){
...
.sd-filter-row th{padding:4px 6px;background:#fff;border-bottom:1px solid var(--br)}
.sd-colf{width:100%;font-size:11px;padding:4px 6px;border:1px solid var(--br);border-radius:5px;outline:none;font-weight:400;text-transform:none;box-sizing:border-box}
.sd-colf:focus{border-color:var(--t)}
.sd-colf-r{text-align:right}
</style>
