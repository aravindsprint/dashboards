...
def get_drill_down(drill_type, from_date=None, to_date=None, company=None,
    ...
    """
    Single whitelisted entry-point for all dashboard drill-downs.
    drill_type options:
      customer_items          → SI: items+commercial_name for a customer
      customer_order_items    → SO: items+commercial_name for a customer
      ...
      transaction_items       → SI or SO line items for a specific document
    """
    from_date, to_date = _date_args(from_date, to_date)
    cf = "AND si.company=%s" if company else ""
    cf2 = "AND so.company=%s" if company else ""

    def _p(*extra):
        base = [from_date, to_date]
        if company:
            base.append(company)
        return base + list(extra)

    # ── helper: get commercial_name column ──────────────────────
    def _cn_col(alias="sii"):
        col = _commercial_name_col("`tabSales Invoice Item`")
        if col:
            return f"{alias}.{col}"
        return f"{alias}.item_name"

    def _cn_col_so(alias="soi"):
        col = _commercial_name_col("`tabSales Order Item`")
        if col:
            return f"{alias}.{col}"
        return f"{alias}.item_name"

    # ── 1. customer → SI item codes ─────────────────────────────
    if drill_type == "customer_items":
        ...
        return frappe.db.sql(f"""
            ...
            LEFT JOIN (
                SELECT parent, MAX(sales_person_name) AS sales_person_name
                FROM `tabSales Team`
                WHERE parenttype='Sales Invoice'
                GROUP BY parent
            ) sp ON sp.parent = si.name
            WHERE si.docstatus=1
            ...
            ORDER BY revenue DESC
            LIMIT 20
        """, _p(customer), as_dict=True)

    # ── 2. customer → SO item codes ─────────────────────────────
    if drill_type == "customer_order_items":
        ...
        return frappe.db.sql(f"""
            ...
            LEFT JOIN (
                ...
                GROUP BY parent
            ) sp ON sp.parent = so.name
            WHERE so.docstatus=1
              AND so.transaction_date BETWEEN %s AND %s {cf2}
              AND so.customer = %s
    ...
    # ── 3. commercial_name → item codes + customers ─────────────
    if drill_type == "commercial_name_detail":
        ...
        return frappe.db.sql(f"""
            ...
            ORDER BY revenue DESC
            LIMIT 20
        """, _p(commercial_name), as_dict=True)
    ...
    if drill_type == "state_customers":
        state_col = "c.state" if _col_exists("`tabCustomer`", "state") else "addr.state"
        si_rows = frappe.db.sql(f"""
            ...
            LEFT JOIN `tabDynamic Link` dl
              ON dl.link_doctype='Customer' AND dl.link_name=c.name AND dl.parenttype='Address'
            LEFT JOIN `tabAddress` addr ON addr.name = dl.parent
            WHERE si.docstatus=1
              AND si.posting_date BETWEEN %s AND %s {cf}
              AND COALESCE({state_col}, addr.state) = %s
            GROUP BY si.customer
            ORDER BY revenue DESC
            LIMIT 20
        """, _p(state), as_dict=True)
    ...
    if drill_type == "salesperson_items":
        ...
        return frappe.db.sql(f"""
            ...
            ORDER BY revenue DESC
            LIMIT 20
        """, _p(sales_person), as_dict=True)

    # ── 7. cost_center → customers + sales persons ───────────────
    if drill_type == "cost_center_customers":
        # Append " - PSS" back if the DB stores it that way
        ...
        si_rows = frappe.db.sql(f"""
            ...
            WHERE si.docstatus=1
              AND si.posting_date BETWEEN %s AND %s {cf}
              AND (si.cost_center = %s OR si.cost_center = %s)
            GROUP BY si.customer
            ORDER BY revenue DESC
            LIMIT 20
        """, _p(cc_val, cc_val + " - PSS"), as_dict=True)
        ...
        so_rows = frappe.db.sql(f"""
            ...
            WHERE so.docstatus=1
              AND so.transaction_date BETWEEN %s AND %s {cf2}
              AND (so.cost_center = %s OR so.cost_center = %s)
            GROUP BY so.customer
            ORDER BY order_value DESC
            LIMIT 20
        """, _p(cc_val, cc_val + " - PSS"), as_dict=True)
    ...
    # ── 8. naming_series → recent documents ─────────────────────
    if drill_type == "naming_series_docs":
        return frappe.db.sql(f"""
            SELECT name, customer, posting_date AS date, grand_total, status
            FROM `tabSales Invoice`
            WHERE docstatus=1
              AND posting_date BETWEEN %s AND %s {cf}
              AND naming_series = %s
            ORDER BY posting_date DESC
            LIMIT 20
        """, _p(naming_series), as_dict=True)

    # ── 9. transaction line items ────────────────────────────────
